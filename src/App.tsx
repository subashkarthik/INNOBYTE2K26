...
const AdminDashboard: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const [filter, setFilter] = useState({ event: '', year: '', dept: '' });
  ...
  const filteredRegs = registrations.filter(r => {
    return (
      (!filter.event || r.event_name === filter.event) &&
  ...
  const handleBroadcast = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        alert(`Broadcast sent to ${result.count} participants!`);
  ...
  const exportToCSV = () => {
    const headers = ['Reg ID', 'Name', 'College', 'Dept', 'Year', 'Email', 'Phone', 'Events', 'Date'];
    const rows = filteredRegs.map(r => {
      let evtStr = '';
      try { evtStr = (JSON.parse(r.events_json || '[]') as SelectedEvent[]).map(e => e.name).join(' | '); } catch (_) {}
      return [r.reg_id, r.full_name, r.college_name, r.department, r.year, r.email, r.phone, evtStr, r.created_at];
    });
    ...
    link.setAttribute("download", "innobyte_registrations.csv");
  ...
  return (
    <div className="fixed inset-0 z-100 bg-[#020617] overflow-y-auto p-6 md:p-12">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-12">
          <div>
            <h1 className="text-4xl font-bold text-gradient mb-2">Admin Dashboard</h1>
            <p className="text-slate-400">Manage and monitor INNOBYTE2K26 registrations.</p>
          </div>
          <button onClick={onClose} className="p-3 glass-panel rounded-full hover:bg-white/10 transition-colors">
            <X size={24} />
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
          {/* Trend Chart */}
          <div className="glass-panel p-8 rounded-3xl border border-white/5 h-[400px]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="glass-panel p-8 rounded-3xl border border-white/5">
              ...
              <div className="h-[200px]">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={stats?.byDept || []} innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">
            ...
            <div className="glass-panel p-8 rounded-3xl border border-white/5">
              <h3 className="text-sm font-black uppercase tracking-[0.2em] text-slate-400 mb-8 flex items-center gap-2">
                <BarChart3 size={16} className="text-brand-accent" /> Popular Events
              </h3>
              <div className="h-[200px]">
                <ResponsiveContainer width="100%" height="100%">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
          {[
            { label: 'Total Registrations', value: stats?.total || 0, color: 'text-brand-primary' },
            { label: 'Departments', value: stats?.byDept?.length || 0, color: 'text-brand-secondary' },
            { label: 'Active Events', value: stats?.byEvent?.length || 0, color: 'text-brand-accent' },
          ].map((s, idx) => (
            <div key={idx} className="glass-panel p-6 rounded-2xl border border-white/5">
              <span className="text-[10px] uppercase tracking-widest text-slate-500 block mb-2">{s.label}</span>
              <span className={`text-4xl font-black ${s.color}`}>{s.value}</span>
            </div>
          ))}
          <div className="glass-panel p-6 rounded-2xl border border-white/5 flex flex-col justify-center gap-4">
            <button onClick={() => setIsBroadcastOpen(true)} className="flex items-center gap-3 text-brand-primary font-bold hover:bg-brand-primary/10 p-2 rounded-lg transition-all text-sm">
              <Send size={18} /> Broadcast Email
            </button>
            <button onClick={exportToCSV} className="flex items-center gap-3 text-brand-secondary font-bold hover:bg-brand-secondary/10 p-2 rounded-lg transition-all text-sm">
        ...
        <div className="glass-panel rounded-3xl overflow-hidden">
          <div className="p-6 border-b border-white/10 flex flex-wrap gap-4 items-center justify-between">
            <h2 className="text-xl font-bold text-white">Participant List</h2>
            <div className="flex flex-wrap gap-4">
              <select 
                className="bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm focus:outline-none text-slate-300"
                value={filter.event}
                onChange={e => setFilter({...filter, event: e.target.value})}
              >
                <option value="" className="bg-[#020617]">All Events</option>
                {Array.from(new Set(registrations.map(r => r.event_name))).map(e => (
                  <option key={e} value={e} className="bg-[#020617]">{e}</option>
                ))}
              </select>
              <select 
                className="bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm focus:outline-none text-slate-300"
              ...
              >
                <option value="" className="bg-[#020617]">All Years</option>
                {YEARS.map(y => <option key={y} value={y} className="bg-[#020617]">{y}</option>)}
              </select>
              <select 
                className="bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm focus:outline-none text-slate-300"
              ...
              >
                <option value="" className="bg-[#020617]">All Depts</option>
                {DEPARTMENTS.map(d => <option key={d} value={d} className="bg-[#020617]">{d}</option>)}
              </select>
            </div>
          </div>
              ...
              <tbody className="divide-y divide-white/5">
                {filteredRegs.map(reg => (
                  <tr key={reg.id} className="hover:bg-white/5 transition-colors">
                    <td className="px-6 py-4 font-mono text-brand-primary text-sm font-bold">{reg.reg_id}</td>
                    <td className="px-6 py-4">
                      <div className="font-bold text-slate-200">{reg.full_name}</div>
                      <div className="text-xs text-slate-500">{reg.college_name}</div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-sm text-slate-300">{reg.department}</div>
                      <div className="text-xs text-slate-500">{reg.year} Year</div>
                    </td>
                    <td className="px-6 py-4">
                      {(() => { try {
                        return (JSON.parse(reg.events_json || '[]') as SelectedEvent[]).map((ev, i) => (
                          <div key={i} className="text-sm font-bold text-slate-200">
                            {ev.name}{ev.teamName ? <span className="text-xs text-brand-accent ml-2">({ev.teamName})</span> : null}
                          </div>
                        ));
                      } catch { return <div className="text-xs text-slate-500">-</div>; } })()}
                    </td>
                    <td className="px-6 py-4">
                      {reg.payment_screenshot ? (
                        <a 
                          ...
                          className="w-10 h-10 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center overflow-hidden hover:border-brand-primary transition-colors group"
                        >
                          <img 
                        ...
                        <span className="text-[10px] font-bold text-brand-accent uppercase tracking-widest">No proof</span>
                      )}
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-xs text-slate-300">{reg.email}</div>
                    ...
                    <td className="px-6 py-4 text-right">
                      <button 
                        onClick={() => handleDelete(reg.reg_id)}
      ...
      <AnimatePresence>
        {isBroadcastOpen && (
          <div className="fixed inset-0 z-120 flex items-center justify-center p-6 pb-20">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsBroadcastOpen(false)} className="absolute inset-0 bg-slate-950/80 backdrop-blur-sm" />
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative z-10 w-full max-w-lg glass-panel p-10 rounded-3xl border border-white/10 shadow-2xl">
              <h2 className="text-2xl font-black text-white mb-6 flex items-center gap-3">
                <Send className="text-brand-primary" /> Broadcast Update
              </h2>
              <form onSubmit={handleBroadcast} className="space-y-6">
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500 mb-3">Subject</label>
                  <input required type="text" className="w-full bg-white/5 border border-white/10 rounded-xl px-5 py-3 focus:outline-none focus:border-brand-primary/50 text-sm" placeholder="e.g. Schedule Update" value={broadcastData.subject} onChange={e => setBroadcastData({...broadcastData, subject: e.target.value})} />
                </div>
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500 mb-3">Message (HTML supported)</label>
                  <textarea required rows={6} className="w-full bg-white/5 border border-white/10 rounded-xl px-5 py-3 focus:outline-none focus:border-brand-primary/50 text-sm" placeholder="Your message to all participants..." value={broadcastData.message} onChange={e => setBroadcastData({...broadcastData, message: e.target.value})} />
                </div>
                <div className="flex gap-4">
                  <button type="button" onClick={() => setIsBroadcastOpen(false)} className="flex-1 py-4 text-sm font-bold border border-white/10 rounded-xl hover:bg-white/5 transition-colors">Cancel</button>
                  <button type="submit" disabled={isSending} className="flex-1 btn-primary text-sm uppercase tracking-widest font-black disabled:opacity-50">
